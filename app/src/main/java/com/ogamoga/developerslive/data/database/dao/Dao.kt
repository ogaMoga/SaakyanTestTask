...
interface Dao {
    @Query(
        "SELECT id, url, description " +
              "FROM items NATURAL JOIN links " +
              "WHERE id = :currentId AND type = :sectionType " +
              "LIMIT 1"
    )
    ...
    @Query(
        "SELECT id, url, description " +
              "FROM items NATURAL JOIN links " +
              "WHERE nextId = :currentId AND type = :sectionType " +
              "LIMIT 1"
    )
    ...
    @Query(
        "SELECT id, url, description " +
              "FROM items NATURAL JOIN links " +
              "WHERE nextId IS NULL AND type = :sectionType LIMIT 1"
    )
    ...
    @Query(
        "SELECT * " +
              "FROM links " +
              "WHERE nextId IS NULL AND type = :sectionType " +
              "LIMIT 1"
    )
